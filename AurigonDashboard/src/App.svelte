<script>
  ...
  let actionStatus = {};   // { username: 'pending' | 'completed' | 'failed' }
  let recentActions = [];
  ...
  async function selectMachine(machine) {
    ...
    try {
      ...
      // Build actionStatus map from recent actions
      actionStatus = {};
  ...
  // Poll action status every 10s when a machine is selected
  let pollInterval;
  $: if (selectedMachine) {
    ...
    pollInterval = setInterval(async () => {
      try {
        ...
        // If a pending action is now gone, refresh accounts
          ...
          <tbody>
            {#each filtered as account}
              <tr>
                ...
                <td class="td-actions">
                  {#if actionStatus[account.username] === 'pending'}
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .login-card {
    width: 360px; background: #111318; border: 1px solid #1e2028;
  ...
  .sidebar-footer { padding: 16px 20px; margin-top: auto; border-top: 1px solid #1e2028; }
